...
export default function FeedbackDialog({ params, text }: { params: string | undefined, text: string }) {
  ...
  return (
    <>
      {(params && !closeDialog) && (
        <div 
          className='flex flex-col items-center justify-center rounded-lg p-2 z-2 bg-green-500 shadow-2xl w-3/4 py-10 border border-neutral-700 md:w-1/2 lg:w-1/4 fixed top-1/5'
        >
          <h1>
            {text}
          </h1>
          <button
            ...
            onClick={() => {
              setCloseDialog(true);
              replace(`${pathname}`); // removes the "product_added=true" params out of the URL
            }}
          >
            Dismiss
          </button>
